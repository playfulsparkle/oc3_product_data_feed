<?php
// Heading
$_['heading_title']                = 'Playful Sparkle - Product Data Feed';
$_['heading_categories']           = 'Kategórie asociácií';
$_['heading_authentication']       = 'Overovanie';
$_['heading_tax_deffinitions']     = 'Definície daní';
$_['heading_getting_started']      = 'Začíname';
$_['heading_setup']                = 'Nastavenie Product Data Feed';
$_['heading_troubleshot']          = 'Bežné problémy';
$_['heading_faq']                  = 'Často kladené otázky';
$_['heading_contact']              = 'Kontaktujte podporu';

// Text
$_['text_extension']               = 'Rozšírenia';
$_['text_success']                 = 'Úspech: Úspešne ste upravili Product Data Feed feed!';
$_['text_import_success']          = 'Úspech: Úspešne ste importovali zoznam kategórií Google!';
$_['text_add_category_success']    = 'Úspech: Úspešne ste pridali asociáciu kategórie!';
$_['text_remove_category_success'] = 'Úspech: Úspešne ste odstránili asociáciu kategórie!';
$_['text_edit']                    = 'Upraviť Product Data Feed';
$_['text_import']                  = 'Na stiahnutie najnovšieho zoznamu kategórií Google <a href="https://support.google.com/merchants/answer/160081?hl=sk" target="_blank" rel="external noopener noreferrer" class="alert-link">kliknite sem</a>, a vyberte taxonómiu s číselnými identifikátormi v prostom textovom (.txt) súbore. Potom nahrajte stiahnutý súbor pomocou zeleného tlačidla.';
$_['text_getting_started']         = '<p><strong>Prehľad:</strong> Rozšírenie "Product Data Feed" pre OpenCart 3.x umožňuje používateľom bez námahy exportovať svoje produkty do <a href="https://merchants.google.com" target="_blank" rel="external noopener noreferrer">Google Merchant Center</a> vo formáte XML. Tento zásadný nástroj zvyšuje viditeľnosť produktov na Google Shopping, čím uľahčuje potenciálnym zákazníkom ich objavovanie a nákup.</p><p><strong>Požiadavky:</strong> Na využitie tohto rozšírenia sa uistite, že máte nainštalovaný a nakonfigurovaný OpenCart 3.x+, PHP 7.3. Okrem toho je potrebný aktívny účet Google Merchant Center na efektívne spravovanie vašich produktových zoznamov.</p>';
$_['text_setup']                   = '<ul><li>Stiahnite si najnovší zoznam kategórií Google z <a href="https://support.google.com/merchants/answer/160081?hl=sk" target="_blank" rel="external noopener noreferrer">podpory Google Merchant Center</a>.</li><li>Prepojte kategórie svojho e-shopu s príslušnými kategóriami Google.</li><li>Nakonfigurujte rozšírenie tak, aby preskočilo produkty, ktoré sú vypredané, ak si to prajete.</li><li>Použite ceny bez dane vo feede aktivovaním príslušnej možnosti a prispôsobte definície daní podľa potreby.</li></ul>';
$_['text_troubleshot']             = '<ul><li><strong>Kategórie Google sa nezobrazujú:</strong> Uistite sa, že ste si stiahli najnovší zoznam kategórií Google a importovali ho do svojho systému. To je nevyhnutné pre správne prepojenie medzi vaším e-shopom a Google Merchant Center.</li><li><strong>Žiadny výstup produktového feedu:</strong> Potvrďte, že je rozšírenie Product Data Feed povolené vo vašej administratívnej časti OpenCart. Ak je povolené a stále nevidíte žiadny výstup, skontrolujte nastavenia rozšírenia na prípadné nesprávne konfigurácie.</li><li><strong>Produkty sa nezobrazujú v Google Merchant Center:</strong> Overte, že sú vaše produkty správne kategorizované a že v údajoch o produktoch nie sú žiadne chyby. Uistite sa, že dodržiavate zásady Google Merchant Center týkajúce sa produktových zoznamov.</li></ul>';
$_['text_faq']                     = '<details><summary>Čo je rozšírenie Product Data Feed?</summary><p>Rozšírenie Product Data Feed pomáha používateľom OpenCart 3.x exportovať údaje o svojich produktoch do Google Merchant Center, čím zvyšuje viditeľnosť na Google Shopping.</p></details><details><summary>Ako môžem aktivovať rozšírenie Product Data Feed?</summary><p>Rozšírenie môžete aktivovať z administratívnej časti OpenCart v sekcii Rozšírenia. Uistite sa, že nastavíte potrebné konfigurácie.</p></details><details><summary>Môžem si prispôsobiť svoj produktový feed?</summary><p>Áno, rozšírenie vám umožňuje prispôsobiť rôzne nastavenia, vrátane použitia cien bez dane a definícií daní, čím zabezpečíte, že váš feed vyhovuje vašim potrebám.</p></details><details><summary>Prečo sa môj produktový feed nezobrazuje v Google Merchant Center?</summary><p>Uistite sa, že je rozšírenie Product Data Feed povolené a že sú vaše produktové kategórie správne prepojené s kategóriami Google. Okrem toho skontrolujte, či v konfigurácii vášho feedu nie sú žiadne chyby.</p></details>';
$_['text_contact']                 = '<p>Pre ďalšiu pomoc sa, prosím, obráťte na náš tím podpory:</p><ul><li><strong>Kontakt:</strong> <a href="mailto:%s">%s</a></li><li><strong>Dokumentácia:</strong> <a href="%s" target="_blank" rel="noopener noreferrer">Dokumentácia pre používateľov</a></li></ul>';
$_['text_gbc2c_restore']           = 'Pred importovaním asociácií kategórií Product Data Feed do kategórií obchodu sa uistite, že ste vybrali správny obchod na import. Záložný súbor, ktorý nahrávate (gbc2c_backup_store_x.txt), musí končiť ID obchodu (x), do ktorého sa údaje obnovujú.';

// Column
$_['column_google_category']       = 'Kategória Google';
$_['column_category']              = 'Kategória';
$_['column_action']                = 'Akcia';

// Tab
$_['tab_general']                  = 'Všeobecné';
$_['tab_help_and_support']         = 'Pomoc a podpora';

// Entry
$_['entry_google_category']        = 'Kategória Google';
$_['entry_category']               = 'Kategória';
$_['entry_data_feed_url']          = 'URL dátového feedu';
$_['entry_status']                 = 'Stav';
$_['entry_login']                  = 'Používateľské meno';
$_['entry_password']               = 'Heslo';
$_['entry_skip_out_of_stock']      = 'Preskočiť vypredané';
$_['entry_tax']                    = 'Používanie cien bez dane';
$_['entry_country']                = 'Krajina';
$_['entry_region']                 = 'Oblasť';
$_['entry_tax_rate']               = 'Sadzba dane';
$_['entry_tax_ship']               = 'Doprava dane';
$_['entry_active_store']           = 'Aktívny obchod';
$_['entry_additional_images']      = 'Zahrnúť ďalšie obrázky';
$_['entry_backup_restore']         = 'Názov súboru pre zálohovanie/obnovenie';

// Button
$_['button_backup']                = 'Zálohovať';
$_['button_restore']               = 'Obnoviť';

// Help
$_['help_copy']                    = 'Skopírovať URL';
$_['help_open']                    = 'Otvoriť URL';
$_['help_additional_images']       = 'Povolením tejto možnosti pridáte do svojho Product Data Feed feedu ďalšie obrázky. Majte na pamäti, že povolenie tejto funkcie môže spomaliť proces generovania feedu a zvýšiť veľkosť generovaného XML súboru.';

// Error
$_['error_permission']             = 'Upozornenie: Nemáte oprávnenie upravovať Product Data Feed feed!';
$_['error_store_id']               = 'Upozornenie: Formulár neobsahuje identifikátor obchodu!';
$_['error_currency']               = 'Upozornenie: Vyberte menu zo zoznamu';
$_['error_upload']                 = 'Súbor sa nepodarilo nahrať!';
$_['error_filetype']               = 'Neplatný typ súboru!';
$_['error_tax_country']            = 'Prosím, vyberte krajinu pre daň.';
$_['error_tax_region']             = 'Pole pre daňovú oblasť nemôže zostať prázdne.';
$_['error_tax_rate_id']            = 'Prosím, vyberte sadzbu dane pre daň.';
$_['error_no_data_to_backup']      = 'Žiadne údaje o priradení kategórií na zálohovanie nie sú k dispozícii.';
